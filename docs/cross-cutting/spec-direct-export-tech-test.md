...
await expectWithinLatency(async () => {
  await directExporter.exportToVault(capture);
}, { p95: 50, p99: 100 }); // milliseconds
```
...
export class TimingMock {
  measureLatency<T>(operation: () => Promise<T>): Promise<{ result: T, latency: number }>;
...
const VOICE_FIXTURES = {
  ...
  SPECIAL_CHARS: {
    ...
    content_hash: computeSHA256('Content with "quotes" and \n newlines \t tabs'),
...
*Remember: These tests are your parachute for the critical path (data integrity), not the paint job (UI polish). Focus on behavior that could cause data loss, not implementation details that could change.*