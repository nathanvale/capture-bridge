...
[
  ...
  { "cmd": "capture:text" , "stdin": "", "expectExit": 2, "expectError": "CLI_INPUT_INVALID" },
  { "cmd": "capture:voice ./missing.m4a", "expectExit": 4, "expectError": "CLI_VOICE_FILE_MISSING" }
]
```
...
describe('Capture → List → Export Chain', () => {
  it('handles voice capture through export workflow', async () => {
    ...
    const captureResult = await harness.execCLI([
      'capture', 'voice', 'test-memo.m4a', '--json'
    ])
    ...
    const listResult = await harness.execCLI([
      'ledger', 'list', '--json'
    ])
...
describe('Doctor → Diagnosis → Recovery Chain', () => {
  it('detects issues and guides recovery', async () => {
    ...
    const doctorResult = await harness.execCLI([
      'doctor', '--json'
    ])
    ...
    const fixResult = await harness.execCLI([
      'doctor', '--fix', '--json'
    ])
...
describe('Init → Configure → First Capture Chain', () => {
  it('guides new user through complete setup', async () => {
    ...
    const firstCaptureResult = await harness.execCLI([
      'capture', 'voice', 'first-memo.m4a', '--json'
    ])
...
describe('Capture Failure Recovery Chain', () => {
  it('handles capture failures with guided recovery', async () => {
    ...
    const failedCaptureResult = await harness.execCLI([
      'capture', 'voice', 'nonexistent.m4a', '--json'
    ])
    ...
    const retryResult = await harness.execCLI([
      'capture', 'voice', path.join(harness.tempVault.path, 'valid-memo.m4a'), '--json'
    ])
...
describe('JSON Output Contract Chains', () => {
  it('maintains stable contracts across command boundaries', async () => {
    ...
    const inspectResult = await harness.execCLI([
      'ledger', 'inspect', captureOutput.id, '--json'
    ])
    ...
    const exportResult = await harness.execCLI([
      'capture:file', captureOutput.id, '--json'
    ])
...
describe('CLI Performance Regression Detection (P1)', () => {
  test('detects p95 latency regression for command startup', async () => {
    ...
    console.log(`Command startup P95 latency: ${p95}ms (baseline: ${BASELINE_P95}ms)`)
  })
  ...
  test('detects p95 latency regression for list query operations', async () => {
    ...
    expect(p95).toBeLessThan(REGRESSION_THRESHOLD)
    console.log(`List query P95 latency: ${p95}ms (baseline: ${BASELINE_P95}ms)`)
  ...
  test('detects p95 latency regression for single capture export', async () => {
    ...
    expect(p95).toBeLessThan(REGRESSION_THRESHOLD)
    console.log(`Export command P95 latency: ${p95}ms (baseline: ${BASELINE_P95}ms)`)
  ...
  test('detects throughput regression for batch operations', async () => {
    ...
    console.log(`Batch processing throughput: ${throughput.toFixed(1)} ops/min (baseline: ${BASELINE_THROUGHPUT})`)
  ...
  test('detects memory leak during sustained CLI usage', async () => {
    ...
    console.log(`CLI heap growth: ${(heapGrowth / 1024 / 1024).toFixed(2)}MB for 100 commands`)
...
describe('CLI Sustained Load (P1)', () => {
  test('handles 1000 commands over 10 minutes', async () => {
      ...
      operation: async (iteration) => {
        ...
        switch (commandType) {
          ...
          case 1:
            ...
            break
          case 2:
            result = await harness.run(`adhd capture --text "Test ${iteration}"`)
            break
          case 3:
    ...
    console.log(`Sustained CLI load: ${firstHalfP95.toFixed(2)}ms → ${secondHalfP95.toFixed(2)}ms P95`)
  ...
  test('maintains system integrity under sustained CLI load', async () => {
    ...
    const concurrentPromises = Array.from({ length: 5 }, async (_, threadId) => {
      ...
      for (let i = 0; i < 100; i++) {
        ...
        try {
          const result = await harness.run(`adhd capture --text "Thread ${operationId}"`)
...
describe('CLI Burst Load (P1)', () => {
  test('handles 100 commands in 10 seconds', async () => {
    ...
    while (Date.now() < endTime && operationCount < 100) {
      ...
      try {
        const result = await harness.run(`adhd capture --text "Burst test ${operationCount}"`)
    ...
    console.log(`Burst CLI load: ${results.length} operations, P95: ${p95.toFixed(2)}ms`)
...
describe('CLI Resource Exhaustion (P1)', () => {
  test('handles graceful degradation approaching system limits', async () => {
    ...
    for (let i = 0; i < 200; i++) {
      ...
      try {
        ...
        if (memoryAfter > 400 * 1024 * 1024) { // Approaching limit
    ...
    const doctorOutput = JSON.parse(doctorResult.stdout)
    expect(doctorOutput.checks.find(c => c.name === 'database').status).toBe('ok')
  ...
  test('handles disk space exhaustion gracefully', async () => {
    ...
    const diskCheck = doctor.checks.find(c => c.name.includes('disk') || c.name.includes('space'))
...
class CLILoadTestHarness extends CLIChainTestHarness {
  ...
  setResourceLimits(limits: { memory?: number, disk?: number, processes?: number }) {
    this.resourceLimits = limits
  }

  async runConcurrentCommands(commands: string[], maxConcurrency = 10): Promise<CLIResult[]> {
  ...
  async checkSystemHealth(): Promise<{ stable: boolean, issues: string[] }> {
    ...
    try {
      ...
      if (this.resourceLimits.memory && resources.memory > this.resourceLimits.memory * 0.9) {
