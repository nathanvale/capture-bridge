...
describe('Performance Benchmarks', () => {
  ...
  describe('Memory Leak Detection', () => {
    ...
    it('should not leak with withTimeout operations', async () => {
      ...
      console.log(`✅ Memory growth from 100 withTimeout calls: ${(growth / 1024).toFixed(2)} KB`);
    }, 30000);
    ...
    it('should not leak with retry operations', async () => {
      ...
      console.log(`✅ Memory growth from 100 retry calls: ${(growth / 1024).toFixed(2)} KB`);
    }, 30000);
  });
  ...
  describe('Concurrent Operations Performance', () => {
    it('should handle 1000 concurrent process mocks in under 100ms', async () => {
      ...
      const mocks = await Promise.all(
        Array.from({ length: 1000 }, (_, i) =>
          Promise.resolve(createMockFn(() => `mock-${i}`))
        )
      );
      ...
      console.log(`✅ Created 1000 concurrent mocks in ${elapsed}ms`);
    });
    ...
    it('should handle connection pool under heavy load', async () => {
      ...
      console.log(`✅ 100 concurrent pool operations in ${elapsed}ms`);
      console.log(`   Pool stats: ${stats.totalConnections} connections, ${stats.hitRate.toFixed(2)}% hit rate`);
    }, 10000);
    ...
    it('should handle concurrent database operations efficiently', async () => {
      ...
      console.log(`✅ 1000 concurrent DB operations across 10 databases in ${elapsed}ms`);
      console.log(`   Average: ${(elapsed / 1000).toFixed(2)}ms per operation`);
    }, 10000);
  });
  ...
  describe('Efficiency Benchmarks', () => {
    it('should verify object pool reuse efficiency', async () => {
      ...
      // Calculate hit rate (connections reused / total acquisitions)
      const totalAcquisitions = stats.connectionsCreated + (stats.connectionsReused || 0);
      const hitRate = totalAcquisitions > 0 ? ((stats.connectionsReused || 0) / totalAcquisitions) * 100 : 0;

      // Most acquisitions should be from reused connections
      if (stats.connectionsReused) {
        expect(stats.connectionsReused).toBeGreaterThanOrEqual(iterations - 5);
      }

      console.log(`✅ Object pool reuse efficiency:`);
      console.log(`   Hit rate: ${hitRate.toFixed(2)}%`);
      console.log(`   Connections created: ${stats.connectionsCreated}`);
      console.log(`   Connections reused: ${stats.connectionsReused || 0}`);
      console.log(`   Total acquisitions: ${totalAcquisitions}`);
    });
    ...
    it('should verify resource cleanup performance', async () => {
      ...
      const dbCount = 50;
      const dbPromises = Array.from({ length: dbCount }, (_, i) =>
        createFileDatabase(`cleanup-perf-${i}.db`)
      );
      ...
      console.log(`✅ Cleaned up ${dbCount} databases in ${elapsed}ms`);
      console.log(`   Average: ${(elapsed / dbCount).toFixed(2)}ms per database`);
    }, 10000);
  });
  ...
  describe('Stress Tests', () => {
    it('should handle rapid connection acquisition and release cycles', async () => {
      ...
      console.log(`✅ ${cycles} rapid acquire/release cycles in ${elapsed}ms`);
      console.log(`   Average: ${(elapsed / cycles).toFixed(2)}ms per cycle`);
    ...
    it('should maintain performance under queue pressure', async () => {
      ...
      console.log(`✅ 20 operations with 2 connections in ${elapsed}ms`);
      if (stats.maxQueueDepth !== undefined) {
    ...
    it('should handle mixed workload patterns efficiently', async () => {
      ...
      const operations = [
        ...
        ...Array.from({ length: 50 }, () =>
          withTimeout(delay(10).then(() => 'done'), 100)
        ),
      ];
      ...
      console.log(`✅ 200 mixed operations in ${elapsed}ms`);
      console.log(`   Average: ${(elapsed / 200).toFixed(2)}ms per operation`);
    }, 10000);
  });
  ...
  describe('Scalability Tests', () => {
    it('should scale linearly with connection pool size', async () => {
      ...
      expect(results[3].elapsed).toBeLessThanOrEqual(results[0].elapsed * 1.5); // Allow some variance

      console.log('✅ Pool size scalability:');
      for (const { poolSize, elapsed } of results) {
        console.log(`   Pool size ${poolSize}: ${elapsed}ms`);
      }
    }, 30000);
    ...
    it('should maintain performance with large datasets', async () => {
      ...
      console.log(`   Insert 10,000 records: ${insertElapsed}ms`);
      console.log(`   Query count: ${queryElapsed}ms`);
    }, 10000);
  });
  ...
  describe('Regression Tests', () => {
    it('should not degrade with repeated test runs', async () => {
      ...
      const degradation = ((runTimings[9] - runTimings[0]) / runTimings[0]) * 100;
      expect(Math.abs(degradation)).toBeLessThan(50); // Less than 50% variation

      console.log(`✅ Performance consistency across 10 runs:`);
