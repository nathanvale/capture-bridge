...
describe('Security Validation', () => {
  ...
  describe('SQL Injection Prevention', () => {
    ...
    it('should sanitize batch operations', async () => {
      const { seedWithBatch, validateBatch } = await import('@orchestr8/testkit/sqlite');
      const Database = (await import('better-sqlite3')).default;
      ...
      // Attempt batch operations with potential SQL injection
      const maliciousOperations = [
        {
          sql: "INSERT INTO products (name, price) VALUES ('Safe Product', 9.99)",
          label: 'Safe operation'
        },
        {
          // This should be handled safely
          sql: "INSERT INTO products (name, price) VALUES (''; DROP TABLE products; --', 19.99)",
      ...
      try {
        const validatedOps = validateBatch(
          maliciousOperations.map(op => ({ type: 'command' as const, value: op.sql }))
        );
        expect(validatedOps).toBeDefined();

        console.log('✅ Batch operations validated');
      } catch (error) {
        // If validation rejects malicious operations, that's good
        console.log('✅ Malicious batch operation rejected');
      }
    ...
    it('should validate migration SQL', async () => {
      ...
      await fs.writeFile(
        path.join(tmpDir, '001_safe.sql'),
        'CREATE TABLE users (id INTEGER PRIMARY KEY, name TEXT);'
      );

      await fs.writeFile(
        path.join(tmpDir, '002_malicious.sql'),
        "CREATE TABLE temp (id INTEGER); DROP TABLE users; --"
      );
  ...
  describe('Path Traversal Prevention', () => {
    ...
    it('should handle null bytes in paths', async () => {
      ...
      const nullBytePaths = [
        'safe.txt\x00malicious.exe',
        'config\x00/etc/passwd',
        'file.txt\x00.dangerous'
      ];
  ...
  describe('Resource Exhaustion Prevention', () => {
    it('should enforce connection limit', async () => {
      ...
      const dbPath = path.join(tmpDir, 'limited.db');
    ...
    it('should enforce file descriptor limits', async () => {
      const { createFileDatabase, registerDatabaseCleanup, cleanupAllSqlite } = await import('@orchestr8/testkit/sqlite');
      const Database = (await import('better-sqlite3')).default;
      ...
      const createdDatabases: Array<{ db: Database; cleanup: () => Promise<void> }> = [];
  ...
  describe('Integration: Combined Security Tests', () => {
    it('should handle complex attack scenarios safely', async () => {
      const { validatePath, sanitizeSqlIdentifier, validateCommand, SecurityValidationError } = await import('@orchestr8/testkit/utils');
      const Database = (await import('better-sqlite3')).default;
    ...
    it('should maintain security under concurrent operations', async () => {
      const { validatePath, ConcurrencyManager, SecurityValidationError } = await import('@orchestr8/testkit/utils');
      ...
      const results = await manager.map(paths, async (testPath) => {
        try {
          return { path: testPath, valid: true, result: validatePath('/tmp', testPath) };
        } catch (error) {
          return { path: testPath, valid: false, error };
        }
      });

      // Verify malicious paths were rejected
      const maliciousPaths = results.filter(r =>
        r.path.includes('..') || r.path.startsWith('/')
      );
      ...
      const safePaths = results.filter(r =>
        !r.path.includes('..') && !r.path.startsWith('/')
      );
