...
Object.entries(phases).sort().forEach(([p, count]) => {
...
Object.entries(risks).sort().forEach(([r, count]) => {
...
let missingFields = 0;
const requiredFields = ['task_id', 'capability_id', 'phase', 'slice', 'title', 'description',
                       'acceptance_criteria', 'risk', 'est', 'depends_on_tasks', 'gap_codes',
                       'provisional', 'related_specs', 'related_adrs', 'related_guides',
                       'parallel', 'conflicts_with', 'file_scope', 'parallelism_group'];
