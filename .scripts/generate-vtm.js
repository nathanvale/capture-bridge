...
const capGraph = JSON.parse(fs.readFileSync(path.join(__dirname, '..', '.generated', 'capabilities.json'), 'utf8'));
...
function analyzeParallelSafety(task, capability) {
  ...
  if (task.title.toLowerCase().includes('monorepo') ||
  ...
  const parallel = capability.risk !== 'High' &&
...
function decomposeCapability(capability) {
  ...
  for (let i = 0; i < taskCount; i++) {
    ...
    const taskContext = {
      introducesPersistence: capability.category === 'storage' || capability.category === 'foundation',
...
const manifestHash = 'sha256:' + crypto.createHash('sha256').update(manifestContent).digest('hex').substring(0, 11);
